...
1. ~~Produce different salts for each context.~~ Done
...
The following are ideas we had while rotating the secrets. Some of these will not be relevant (or will be different) if we move to Vault.

### Metadata
Add metadata to our keys (ex: a key id). This would allow us to log the metadata rather than the last digits of the secret.

### How do we store and use secrets?
In general we should probably store secrets differently. SRE is also interested in creating some CDS standards for storing and using secrets, possibly using Vault.

### LastPass
Drop as many secure notes as possible from LastPass such as storing the prod/staging .env files which are already in the manifest repository.

### GitHub secrets
Currently there’s a lot of manually updating GitHub action secrets. This can cause errors and are not possible to cleanly revert. Hopefully Vault will fix this.

### Auth Tokens
AUTH_TOKENS was a list of 2 tokens that document-download-api will accept to authenticate api.
DOCUMENT_DOWNLOAD_API_KEY is the first in the list and is what api uses
We should simplify this (perhaps, just have one variable DD_API_AUTH_TOKEN )

### Bulk-send
We have a special way to upload 100K row csvs directly to s3 and send. This is only available to the services: Load Testing, Get Updates on COVID-19, and Obtenir les nouvelles sur la COVID-19.
It is believed that this capability was never actually used. 
This was put in place before the /bulk endpoint, which now essentially provides this capability. 
Getting rid of this would clean up code in the api, admin, and terraform repos.

### Inspec-scanner
This IAM user was used by the now archived guardrail scanning product. SRE has said that we can remove this user.

### Api-lambda
The api lambda gets most secrets from the parameter store / manifest repo. A few are set in the lambda configuration from the terraform repo. We should move as many as possibly to the manifest repo (in particular if there are duplicates)

### IAM Users
We should be able to move most of these to OIDC
We have started this process with the AWS_ECR_ACCESS_KEY_ID / AWS_ECR_SECRET_ACCESS_KEY that was used to push to CDS’s public repo

### Database users / passwords:
Currently everything shares the same database user. We should consider making different users for each application (app, hasura, blazer, maybe even split k8s and api lambda). This would more keys, but each one easier to rotate.
