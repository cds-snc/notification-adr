...
$$Bounce Rate = \frac{\sum\limits_{n=1}^{24} \text{hard bounces n} +\text{length}( \text{hard bounces:\{service id\}} )}{\sum\limits_{n=1}^{24} \text{total notifications n} + \text{length}( \text{total notifications:\{service id\}} )}$$
