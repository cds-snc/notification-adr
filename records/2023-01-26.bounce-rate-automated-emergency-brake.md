...
`total_notifications:{service_id}`\
`hard_bounces:{service_id}`

$$\text{Bounce Rate} = \frac{\text{length}( \text{hard\_bounces:\{service\_id\}} )}{ \text{length}( \text{total\_notifications:\{service\_id\}} )}$$


The reason we put a `{k:v}` as epoch is because the `k` must be unique. We add to total_notifications every single time we send a notification. We keep the key as `epoch` so we might calculate the bounce rate using a sliding window. We can find the length of the above two keys, where only the keys in the last 24 hours (this is adjustable) are included in the calculation.

### Seeding Redis

#### Seeding the keys:
If we have to seed `total_notifications` or `hard_bounces` per service_id, we would have to take every single notification added in the last X-hours (X being 24 etc) and add to the total_notification_service_id. This means looping through every single notification row for the last 24 hours and adding it to the list.

Instead we will find the total_notification and hard_bounces for each of the last 24 hours and add that to redis. For each `service_id` we will now have 50 keys - 48 keys, one for total_notifications / hour and one of hard_bounces/ hour.

The following keys:\
`total_notifications_service_id_{24_hours}`\
`total_notifications_service_id_{23_hours}`\
....\
`hard_bounces_service_id_{1_hour}`\
....\
`total_notifications:{service_id}`\
`hard_bounces:{service_id}`

The 48 keys would expire within the 24 hour mark (or whatever we chose the sliding window to be)

The new bouce rate calculation will be:

$Bounce Rate = \frac{\sum\limits_{n=1}^{24} \text{hard\textunderscore bounces\textunderscore n} +\text{length}( \text{hard\textbackslash text\_bounces:\{service\textbackslash text\_id\}} )}{\sum\limits_{n=1}^{24} \text{total\textunderscore notifications\textunderscore n} + \text{length}( \text{total\textbackslash text\_notifications:\{service\textbackslash text\_id\}} )}$
...
If we decide to seed the bounce rate in any given case, we will have an AWS ECS Task that takes the above data from the DB and writes it to Redis.
