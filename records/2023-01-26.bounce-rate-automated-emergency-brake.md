...
$$\text{Bounce Rate} = \frac{\sum\limits_{n=1}^{24} \text{hard\_bounces\_{n}} + \text{length}( \text{hard\_bounces:\{service\_id\}} )}{\sum\limits_{n=1}^{24} \text{total\_notifications\_{n}} +  \text{length}( \text{total\_notifications:\{service\_id\}} )}$$
