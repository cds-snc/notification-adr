...
A synchronous bounce occurs during the call to send the email with the AWS SES API. An asynchronous bounce occurs after the email has been sent to SES and is communicated via a callback to Notify.
...
In order to store the bounce rate GC Notify needs to record the bounce type of every notification.  This will require a new column in the notifications table to store the bounce type.  
...
Bounce rates can be monitored by a periodic celery task that polls the database for bounce rates, and notifies services that are over `[bounce-rate-warning-percentage]` and suspends services that are over `[bounce-rate-critical-percentage]`.  This task should be run several times a day to ensure that services are notified and suspended as soon as possible after they reach one of these limits.  A query to get service bounces rates could look something like this:
```sql
with data as
(
...
### How GC Notify handles asynchronous bounce notifications from SES
...
### How GC Notify handles synchronous bounce notifications from SES
GC Notify sends emails via the `send_email()` method in `clients/email/aws_ses.py`.  
- :warning: There does not appear to be code to record asynchronous bounces in this method.
...
|-------------------------------------------------|----------|------------------------------------------------------------------------------------------------------------------------------|
| bounce-rate-warning-percentage                  |       4% | AWS indicates a 5% bounce rate as under review, so we should warn before this point                                          |
| bounce-rate-critical-percentage                 |       5% | AWS indicates a 5% bounce rate as under review, so we should suspend at this point                                           |
| bounce-rate-window                              | 24 hours | This should be at a minimum 24 hours, but could be a longer period                                                           |
| bounce-rate-warning-notification-volume-minimum |     2500 | This should be a significantly high number to ensure we only notify users with enough volume to sway the overall bounce rate |
