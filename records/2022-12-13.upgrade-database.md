...
### Option C_2: Blue / Green migration
This is similar to Option C, but has built in support in AWS. AWS labels the original database "blue" and creates a copy labeled "green". New data will get copied to green. We then perform a switchover from blue to green. This approach requires us to first move to 11.21 and disable the RDS proxy.
...
After much discussion and testing we decided to proceed with Option C_2, the built in blue/green migration.

## Process

We first upgrade to 11.21, which required approximately 2 minutes of downtime.

Scripts were created to perform the following steps:
1. Move off the RDS proxy and delete it.
2. Create the blue/green deployment, with the green database using PostgreSQL 15.5
3. Switch from using the blue to using the green.
4. Tear down the blue/green deployment (leaving us with a single database).
5. Recreate the RDS proxy and point applications to it.
