# Ugrade GC Notify Postgres to Latest Major Version
...
To minimize risk, GC Notify could ugrade to Postgres 12.x in the short term. This would provide an additional year of being within the standard AWS support window, as the end of life target for Aurora Postgres 12.x is February 28th 2025. 
...
Ugrading to Postgres 15 without causing significant downtime required the use of the blue/green deployment concept offered by AWS. A typcial blue/green deployment does the following:

1. AWS creates a Blue/Green Deployment group in RDS
2. AWS associates the existing database cluster to this deployment group as the "Blue" database
3. AWS creates a new database cluster under the deployment group using the existing database's data and labels it the "green" database
4. AWS configures logical replication between the blue and green databases so that all new data to blue is automatically sent to green. **At this point only the blue database is still in active use**
5. AWS begins the upgrade process on the green database to ugrade to the desired version
6. When complete, the notify team can connect applications to the green database manually to test the upgrade
7. Once testing is completed, the Notify team can trigger a switchover between blue and green.
8. When switchover is initiated, AWS cuts traffic to the blue database and waits for all replication to complete in the green database. **There is a short amount of downtime of 10-15 seconds associated with this step**
9. When replication is complete, AWS enables the green database as the active database
10. AWS renames the blue database, appending "-old" to the name
11. The Notify team deletes the blue/green deployment group, which deletes the "old" blue database


### Problems

There are two problems with the blue/green upgrade specific to Noify:
...
2. The blue/green deployment strategy does not support the usage of an RDS proxy. Notify uses an RDS proxy to improve database performance, since the RDS proxy is better able to manage a large number of concurrent connections to the database from the celery workers. It will be necessary to remove the RDS Proxy temporarily so that the blue/green deployment can be performed.
...
Notify Core will make a release that sets the enable flag on logical repclication, and schedule a restart of the database several days before performing the blue/green deployment upgrade. This results in approximately 10 seconds of downtime
...
A shell command is run outside of Terraform to create the blue/green deployment. The reason this is done outside of terraform is that this is an ephemeral step, and Terraform does not support this use case.
...
Notes:
- in step 3 and 5 there were a few notifications that stalled in "sending" before being retried a few minutes later (5 for step 3, 2 for step 5)
...
- these stuck notificaitons will be sent after a 4 hour, 15 minute delay. This may be unacceptable; if so we should decide how to reduce the delay.
