...
The scan-files project comes with a terraform module that triggers a scan of newly created S3 objects in a bucket it is told to watch. The S3 bucket we are currently using to store file attachments won't work, since these file are each encrypted with a unique key and scan-files tf module doens not support this. But we could perhaps create a duplicate S3 bucket with restricted access containing unencrypted versions of these files.

That would work but we would need to be able to map files from one S3 bucket to the other, and we would need to enforce the same retention/deletion policies on both buckets.
