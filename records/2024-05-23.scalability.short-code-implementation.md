...
There were no surprises. The solution worked as expected. This lighthened our load for picking the right priority lane for each template, now determined automaticlaly by the asssociated priority lane for the template category. Alos, the automatic categorization of the existing templates with their corresponding priority lane worked pretty well. The override for the priority lane over the template category is also a nice option to have when a template does not entirely fit based on its usage even though the category is correctly assigned. We have a current case where this applied.
