...
While the addition of magic links provides users with a less cumbersome, more accessible and convenient login experience there are a number of security implications that we identified during a [threat modeling exercise](https://docs.google.com/document/d/1sIdrJ_dlkzwQoEVss0SMXjhdrLPYUenFlf3Xm8-amUc) that we should mitigate.
...
## General Mitigations
* Currently our 2FA codes are [valid for thirty minutes](https://github.com/cds-snc/notification-admin/blob/main/app/config.py#L75). Lowering this by fifteen minutes or more is a reasonable mitigation that could apply regardless of other chosen mitigation strategies.
* See the [Fallback Mitigation section](#Fallback-Mitigation---An-over-engineered-"last-resort"-solution)
...
### Replace the `SystemRandom` module with the `secrets` module
Currently we use the [`SystemRandom`](https://docs.python.org/3/library/random.html) module to [generate our 2FA codes](https://github.com/cds-snc/notification-api/blob/main/app/dao/users_dao.py#L23). However it is not advised to use this module for security purposes. We need to refactor the 2FA code to leverage the `secrets` module to do this instead, which is designed specifically with cryptographic strength suitable for passwords, secrets, security tokens, etc.
